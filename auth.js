...
function renderUsuarios(){
  ...
  el.innerHTML=`<h2 style="color:var(--navy);border-bottom:2px solid var(--gold);padding-bottom:6px">👥 Usuários</h2>
    ...
    <div class="tblwrap"><table class="cfbtable"><thead><tr><th>Nome</th><th>E-mail</th><th>Papel</th><th>Status</th><th></th></tr></thead><tbody id="usuList"><tr><td colspan="5" style="padding:14px;color:#999">Carregando…</td></tr></tbody></table></div>`;
    ...
    tb.innerHTML = rows.length? rows.map(u=>{
      ...
      return `<tr>
        ...
        <td>${me?'':(u.ativo
            ...
            : `<button class="btn g sm" onclick="usuSetAtivo('${u.uid}',true)">Liberar</button>`)}</td>
      </tr>`;
    }).join('') : '<tr><td colspan="5" style="padding:14px;color:#999">Nenhum usuário ainda.</td></tr>';
  }).catch(e=>{ const tb=document.getElementById('usuList'); if(tb) tb.innerHTML='<tr><td colspan="5" style="padding:14px;color:#c0392b">Erro ao listar: '+(e&&e.message||e)+'</td></tr>'; });
}
